...
def quick_sort_asc(array):
    ...
    def partition(array, lo, hi):
        ...
        i = lo - 1  # przedział poza żeby zgadzało sie w pętli while
        j = hi + 1  

        while True:
            # Przesuń i w prawo, dopóki nie znajdziesz wartości >= pivot (większej równej)
            while True:
                i += 1
                if array[i] >= pivot:
                    break
            # Przesuń j w lewo, dopóki nie znajdziesz wartości <= pivot (mniejszej równej)
            while True:
                j -= 1
                if array[j] <= pivot:
                    break
    ...
    def recursive_quicksort(array, lo, hi):
        if lo >= hi:# kiedy sie miną to koniec rekurencji
            return 
...
quick_sort_asc(arr)
print("Posortowana tablica rsonąco:", arr)
